...
app.post('/login', function (req, res) {
  ...
  connection.query(`SELECT * from users where username="${usr.uname}"`, function(err, rows, fields) {
    if (!err) {
...
app.post('/register', function (req, res) {
  ...
  const { firstname, lastname, email, gender , nationality, contact } = req.body;
  connection.query(`SELECT * from users where username="${usr.uname}"`, function(err, rows, fields) {
    if(rows.length <= 0){
      //no user with specified username
      connection.query('INSERT INTO users SET username = ?,password = ?, firstname = ?, lastname = ?, email = ?, gender = ?, nationality = ?, contact = ?', [usr.uname, pwwd, firstname, lastname, email, gender, nationality, contact], (err, rows) => {
  
     // connection.query(`INSERT into user(username, password, email, gender, nationality, contact) values("${usr.uname}","${pwwd}","${email}","${gender}","${nationality}","${contact}")`, function(err, rows, fields) {
        if(!err) {
          res.redirect('/login');
        } else {
...
// news
app.get('/videos', function (req, res, error) {
  res.locals.user = req.session.user;


   
    res.render('videos', {
      title : "Olympic 2022 games",
...
//add news
app.post('/admin/addnews', function (req, res) {
  res.locals.user = req.session.user;
  const { title,description} = req.body;
  connection.query('Insert into news set title = ?,description = ?',[title, description], function (error, results, fields) {
    if (error) throw error;
    res.render('admin/news', {
      title : "Olympic 2022 news",
      news: results
    })
  });
...
app.get('/video', function(req, res){
  ...
  const videoPath = "./olympic_live.mp4";
